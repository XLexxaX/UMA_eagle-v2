...
//		Mapping f = new Mapping();
        ...
        result = aLD.getControversyCandidates(mapList, 1, null);
        System.out.println(result);
        aLD.setKnown(MappingFactory.createDefaultMapping());
        aLD.retrieved.clear();

        result = aLD.getControversyCandidates(mapList, 2, null);
        System.out.println(result);

        result = aLD.getControversyCandidates(mapList, 8, null);
        System.out.println(result);
    }
     ...
     * @return
     */
    public List<Triple> getControversyCandidates(List<AMapping> mapList, int n, GoldStandardBatchReader gsbr) {
    	...
    	//                            retrieved.add(t); // remember the triple, to don't ask about it twice
                            ...
                            if (!retrievedIDs.contains(t.getSourceUri()) && !gsbr.isContained(t.getSourceUri(), t.getTargetUri(), gsbr.getTrain())) {
                            		t.setSimilarity((float) MapsForFound / (float) mapList.size());
    	                            tripleList.add(t);
    	//                            retrieved.add(t); // remember the triple, to don't ask about it twice
    	                            retrievedIDs.add(t.getSourceUri());
