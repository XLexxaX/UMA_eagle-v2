...
//		System.gc();
    ...
    public double calculateRawMeasure(IGPProgram p) {
        ...
        try {
        	if(!useFullCaches)
        		actualMapping = getTrimmedMapping(trimmedSourceCache, trimmedTargetCache, spec);
        	else
        		actualMapping = getMapping(sC, tC, spec);
        } catch (java.lang.OutOfMemoryError e) {
            e.printStackTrace();
    ...
    public AMapping getTrimmedMapping(ACache trimmedSourceCache, ACache trimmedTargetCache, LinkSpecification spec) {
        try {
            ...
            logger.error("Exception execution expression " + spec+" on Caches "+trimmedSourceCache.size()+", "+trimmedTargetCache.size());
    ...
    public ACache getTrimmedSourceCache() {
		return trimmedSourceCache;
	}

	public void setTrimmedSourceCache(ACache trimmedSourceCache) {
		this.trimmedSourceCache = trimmedSourceCache;
	}

	public ACache getTrimmedTargetCache() {
		return trimmedTargetCache;
	}

	public void setTrimmedTargetCache(ACache trimmedTargetCache) {
		this.trimmedTargetCache = trimmedTargetCache;
	}
