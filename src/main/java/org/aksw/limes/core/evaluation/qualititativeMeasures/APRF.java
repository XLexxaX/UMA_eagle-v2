...
public abstract class APRF implements IQualitativeMeasure {
    ...
    public static double trueFalsePositiveReversed(final AMapping predictions, final AMapping goldStandard, boolean truePositive) {

    	double TPCounter = 0, FNCounter = 0;
        for (String sUri : goldStandard.getMap().keySet()){
            for (String tUri : goldStandard.getMap().get(sUri).keySet()){
                if (predictions.contains(sUri, tUri) && goldStandard.getMap().get(sUri).get(tUri) > 0.0 && predictions.getMap().get(sUri).get(tUri) > 0.0){
                    TPCounter++;
                } else if(!predictions.contains(sUri, tUri) && goldStandard.getMap().get(sUri).get(tUri) > 0.0){
                    FNCounter++;
                }
            }
        }
