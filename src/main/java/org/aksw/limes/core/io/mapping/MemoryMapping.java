...
public class MemoryMapping extends AMapping{
    ...
    public boolean contains(String sourceInstance, String targetInstance) {
    	...
    	if (map.containsKey(targetInstance)) {
            if (map.get(targetInstance).containsKey(sourceInstance)) {
                return true;
            }
        } 
        return false;
	 ...
	 * @return number of mappings with value greater than 0
	 */
	public int getNumberofPositiveMappings(AMapping ref) {
		int size = 0;
		for (String s : map.keySet()) {
			for (String k : map.get(s).keySet()) {
				if (map.get(s).get(k) > 0 && ref.contains(s, k)) {
					size++;
				}
			}
		}
